...
def extract_data(df_dialogs):
  ...
  for i in tqdm.trange(len(df_dialogs)):
    if df_dialogs['speaker'][i] == 'T':
      text = "A:" + str(df_dialogs["text"][i])

      emotion.append(str(df_dialogs["emotion"][i]))
      data.append(text)
    else:
      text = "B:" + str(df_dialogs["text"][i])
      emotion.append(str(df_dialogs["emotion"][i]))
      data.append(text) 
  return data, emotion

df = pd.read_csv("PATH_TO_THE_COUNSELING_DATASET")
print(df.describe())

df.head()


data, emotion = extract_data(df)


values=df['conv_id'].unique().tolist()
conv_ids = df['conv_id'].tolist()

# print a sample of the persuasion dialog
dataset = []
conversation = []
emotion_set = []
...
class Counselling_Dataset(Dataset):
    def __init__(self, data, emotion, tokenizer):
...
train_dataset = Counselling_Dataset(train_data, train_emotion, tokenizer)
val_dataset = Counselling_Dataset(val_data, val_emotion, tokenizer)
...
# load the model
model_A = GPT2LMHeadModel.from_pretrained("gpt2-medium", return_dict=False)
model_B = GPT2LMHeadModel.from_pretrained("gpt2-medium", return_dict=False)
...
for ep in range(num_epochs):
    ...
    torch.save([model_A.state_dict(), model_B.state_dict()], f"PATH_TO_SAVE_THE_MODEL_IN_EACH_EPOCH/model_{ep}.pth")
